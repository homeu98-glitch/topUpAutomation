import { applyCors, updateTransactionReviewData } from "../../lib/topup-service.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { transactionId, totalAmount, itemAmounts } = req.body || {};
    if (!transactionId) {
      return res.status(400).json({ error: "缺少交易編號" });
    }

    const data = await updateTransactionReviewData({
      shopId: session.shopId,
      transactionId: String(transactionId),
      totalAmount,
      itemAmounts: Array.isArray(itemAmounts) ? itemAmounts : [],
    });
