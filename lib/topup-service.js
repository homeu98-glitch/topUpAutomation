...
export async function listTransactions({ shopId, mode = "pending", from, to, customerCode, page = 1, pageSize = 20 }) {
  ...
  if (orderNos.length) {
    try {
      ...
      verifiedSet = new Set((verifiedRows || []).map((row) => String(row.pay_order_no)));
    } catch {
      verifiedSet = new Set();
    }
  }
  ...
  return {
    rows: rows.map((row) => ({
      ...row,
      isVerified: Boolean(row?.transaction_OrderNo && verifiedSet.has(String(row.transaction_OrderNo))),
    })),
    total: count || 0,
    page: normalizedPage,
