...
async function compressImage(file) {
  ...
  const width = metadata.width && metadata.width > 1280 ? 1280 : metadata.width;
...
export async function analyzeUploadedFiles(files, memberCode = "") {
  ...
  return buildBatchPayload(items, memberCode);
}
...
export async function listTransactions({ shopId, mode = "pending", from, to }) {
  const client = ensureDatabaseReady();
  let query = client
    .from("transactions")
    .select("*")
...
export async function approveTransaction({ shopId, transactionId, ownerLogin }) {
  ...
  if (error) throw new Error(`核准交易失敗：${error.message}`);
  return data;
}

export async function getDashboardStats({ shopId, from, to }) {
  const rows = await listTransactions({ shopId, mode: "all", from, to });
