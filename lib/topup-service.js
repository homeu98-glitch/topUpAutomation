...
export async function listTransactions({ shopId, mode = "pending", from, to, customerCode, page = 1, pageSize = 20 }) {
  ...
  const orderNos = [
    ...new Set(
      rows
        .flatMap((row) => (row?.items || []).map((item) => item?.extracted?.transactionOrderNo))
        .filter(Boolean)
        .map(String)
    ),
  ];
  ...
  return {
    rows: rows.map((row) => ({
      ...
      items: (row?.items || []).map((item) => {
        const orderNo = item?.extracted?.transactionOrderNo ? String(item.extracted.transactionOrderNo) : "";
        const matchedStatus = orderNo ? mpayStatusByOrderNo.get(orderNo) || null : null;
        return {
          ...item,
          verificationStatus: !orderNo ? null : matchedStatus == null ? "no_match" : matchedStatus === "01" ? "verified" : "invalid",
      ...
      verificationStatus: (() => {
        const statuses = (row?.items || [])
          .map((item) => {
            const orderNo = item?.extracted?.transactionOrderNo ? String(item.extracted.transactionOrderNo) : "";
            const matchedStatus = orderNo ? mpayStatusByOrderNo.get(orderNo) || null : null;
            return !orderNo ? null : matchedStatus == null ? "no_match" : matchedStatus === "01" ? "verified" : "invalid";
          })
          .filter(Boolean);
