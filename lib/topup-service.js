...
10. allDetectedOrderNos 請列出圖片中所有你能辨識到、長度剛好 22 位的純數字候選單號，使用字串陣列，例如 ["2026062411365300000010","2026062411365300000011"]。不要自行補位，不要輸出非 22 位數。
...
function dedupeOrderNoCandidates(values) {
  return [...new Set(values.map((value) => String(value || "").trim()).filter((value) => /^\d{22}$/.test(value)))];
}

function extract22DigitNumbersFromText(text) {
  if (!text) return [];
  const matches = String(text).match(/\d{22}/g) || [];
...
function collectOrderNoCandidates(extracted = {}) {
  ...
  return dedupeOrderNoCandidates([
    extracted?.transactionOrderNo,
    ...(Array.isArray(extracted?.allDetectedOrderNos) ? extracted.allDetectedOrderNos : []),
    ...extract22DigitNumbersFromText(rawText),
  ]);
}
...
function normalizeAnalyzedItem(extracted, file, index, extras = {}) {
  ...
  return {
    ...
    extracted: {
      ...
      transactionOrderNo:
        (extracted.transactionOrderNo && /^\d{22}$/.test(String(extracted.transactionOrderNo))
...
function validateMembershipClaims(claims) {
  const now = Math.floor(Date.now() / 1000);
  const skew = MEMBERSHIP_TOKEN_CLOCK_SKEW_SECONDS;
  const portalRole = String(claims?.portal_role || "");
  const hasShopIdentity = Boolean(claims?.shop_id || claims?.shop_code || claims?.shop_name);

  if (!claims?.iss || claims.iss !== MEMBERSHIP_TOKEN_ISSUER) {
    throw new Error("登入 token issuer 不正確");
  }
  if (!claims?.aud || claims.aud !== MEMBERSHIP_TOKEN_AUDIENCE) {
  ...
  if (!claims?.iat || !claims?.exp || !claims?.nbf) {
    throw new Error("登入 token 缺少時間欄位");
  }
  if (Number(claims.nbf) > now + skew) {
...
export function verifyMembershipLoginToken(token) {
  ...
  const expectedSignature = crypto
    .createHmac("sha256", MEMBERSHIP_TOKEN_SECRET)
  ...
  validateMembershipClaims(parsed.payload);
  return {
...
async function reserveMembershipTokenLogin({ claims, tokenHash, req }) {
  ...
  try {
    const { error } = await client.from("membership_token_logins").insert({
      ...
      member_code: String(claims.member_code),
...
export async function provisionMembershipShop(claims) {
  ...
  const ownerLogin = claims.owner_login
    ...
    : claims.owner_code
      ...
      : `membership-owner-${String(claims.sub).replace(/[^a-zA-Z0-9_-]/g, "").slice(0, 24) || "shop"}`;
  ...
  const insertPayload = {
    code: shopCode || `SHOP-${(externalShopId || crypto.randomUUID()).slice(0, 12)}`,
...
function buildMembershipSyncPayload({ transaction, shop, ownerLogin }) {
  const approvedAt = new Date().toISOString();
  return {
    event: "topup_approved",
    transaction_id: transaction.id,
    member_code: transaction.customer_code,
    ...
    shop: {
      ...
      code: shop.code || null,
      name: shop.name || null,
      external_shop_id: shop.external_shop_id || null,
    },
    items: (transaction.items || []).map((item) => ({
      id: item.id || null,
      merchant_name: item?.extracted?.merchantName || null,
      transaction_order_no: item?.extracted?.transactionOrderNo || null,
      amount: item?.extracted?.amount ? Number(item.extracted.amount) : null,
      transaction_time: item?.extracted?.transactionTime || null,
      payment_method: item?.extracted?.paymentMethod || null,
    })),
  };
}
...
async function syncApprovedTransactionToMembership({ transaction, shop, ownerLogin }) {
  if (!MEMBERSHIP_SYNC_ENDPOINT || !MEMBERSHIP_SYNC_SECRET) {
    throw new Error("會員充值回寫 API 尚未設定完成");
  }

  const payload = buildMembershipSyncPayload({ transaction, shop, ownerLogin });
  const body = JSON.stringify(payload);
  const timestamp = new Date().toISOString();
  const signature = crypto
    .createHmac("sha256", MEMBERSHIP_SYNC_SECRET)
    .update(`${timestamp}.${body}`)
    .digest("hex");

  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), MEMBERSHIP_SYNC_TIMEOUT_MS);

  try {
    const response = await fetch(MEMBERSHIP_SYNC_ENDPOINT, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-Topup-Timestamp": timestamp,
        "X-Topup-Signature": signature,
        "X-Topup-Event": "topup_approved",
      },
      body,
    ...
    if (!response.ok) {
      throw new Error(
        `會員充值回寫失敗：${response.status} ${typeof parsedResponse === "string" ? parsedResponse : JSON.stringify(parsedResponse || {})}`
      );
    }
    ...
    await markMembershipSyncResult({
      ...
      status: "failed",
      syncedAt: null,
      errorMessage: error instanceof Error ? error.message : "membership sync failed",
      responsePayload: null,
    });
    throw error;
  } finally {
...
export async function listTransactions({ shopId, mode = "pending", from, to, customerCode, page = 1, pageSize = 20 }) {
  const client = ensureDatabaseReady();
  ...
  const mpayStatusByOrderNo = new Map();
  if (orderNos.length) {
    try {
      const { data: verifiedRows } = await client
        .from("transactions_from_mpaybackoffice")
        .select("pay_order_no, transaction_status")
        .in("pay_order_no", orderNos);
      (verifiedRows || []).forEach((row) => {
        const key = row?.pay_order_no ? String(row.pay_order_no) : "";
        if (!key) return;
        mpayStatusByOrderNo.set(key, String(row?.transaction_status || ""));
      });
    } catch {
      // ignore
    }
    ...
    rows: rows.map((row) => ({
      ...
      items: (row?.items || []).map((item) => {
        const candidates = [
          ...
          .filter(Boolean)
          .map(String);
        const matchedOrderNo = candidates.find((candidate) => mpayStatusByOrderNo.has(candidate)) || null;
        const matchedStatus = matchedOrderNo ? mpayStatusByOrderNo.get(matchedOrderNo) || null : null;
        return {
          ...item,
          verificationMatchedOrderNo: matchedOrderNo,
          verificationStatus:
            !candidates.length ? null : matchedStatus == null ? "no_match" : matchedStatus === "01" ? "verified" : "invalid",
      ...
      verificationStatus: (() => {
        const statuses = (row?.items || [])
          .map((item) => {
            ...
            const matchedOrderNo = candidates.find((candidate) => mpayStatusByOrderNo.has(candidate)) || null;
            const matchedStatus = matchedOrderNo ? mpayStatusByOrderNo.get(matchedOrderNo) || null : null;
            return !candidates.length ? null : matchedStatus == null ? "no_match" : matchedStatus === "01" ? "verified" : "invalid";
          })
          .filter(Boolean);
...
export async function batchApproveTransactions({ shopId, transactionIds, ownerLogin }) {
  ...
  for (const transactionId of transactionIds) {
    try {
      const data = await approveTransaction({ shopId, transactionId, ownerLogin });
      if (data) {
