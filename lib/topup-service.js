...
export async function createTransactionSubmission({ shopId, customerCode, files, analyzedItems }) {
  ...
  const insertPayload = {
    ...
    status: "pending",
    transaction_OrderNo: extractedOrderNos?.[0] ? String(extractedOrderNos[0]) : null,
    items,
  };
...
export async function listTransactions({ shopId, mode = "pending", from, to, customerCode, page = 1, pageSize = 20 }) {
  ...
  const rows = data || [];
  const orderNos = [...new Set(rows.map((row) => row?.transaction_OrderNo).filter(Boolean).map(String))];
  ...
  return {
    rows: rows.map((row) => ({
      ...row,
      verificationStatus: (() => {
        const orderNo = row?.transaction_OrderNo ? String(row.transaction_OrderNo) : "";
        if (!orderNo) return null;
        if (!mpayStatusByOrderNo.has(orderNo)) return "no_match";
        return mpayStatusByOrderNo.get(orderNo) === "01" ? "verified" : "invalid";
      })(),
      mpayTransactionStatus: (() => {
        const orderNo = row?.transaction_OrderNo ? String(row.transaction_OrderNo) : "";
        if (!orderNo) return null;
        return mpayStatusByOrderNo.get(orderNo) || null;
      })(),
    })),
    total: count || 0,
    page: normalizedPage,
