...
export async function listCustomerTransactions({ shopId = "", customerCode, page = 1, pageSize = 50 }) {
  ...
  let query = client
    .from("transactions")
    .select("id, shop_id, customer_code, total_amount, item_count, status, submitted_at, approved_at, items", {
      count: "exact",
    })
...
export async function approveTransaction({ shopId, transactionId, ownerLogin }) {
  ...
  const syncResult = await syncApprovedTransactionToMembership({
    transaction,
    shop,
    ownerLogin,
  });
  ...
  return rows?.[0]
    ? {
        ...rows[0],
        membershipSyncStatus: "success",
        membershipSyncedAt: syncResult.approvedAt,
      }
    : null;
}
...
export async function runAutoApprovalSweep({ shopId } = {}) {
  const client = ensureDatabaseReady();
  ...
  for (const shop of shops || []) {
    ...
    for (const row of pendingRows || []) {
      ...
      if (!submittedAtMs || now - submittedAtMs < intervalSeconds * 1000) {
        continue;
      }
      try {
